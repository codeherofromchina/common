...
public class BuyerOrgInfo {
	...
	private static final String BASE_INFO_QUERY_SQL = "SELECT t1.buyer_no, t1.buyer_code, t4.id AS org_id, t4.name AS org_name, t1.created_by AS acquiring_user_id , emp.name AS acquiring_user_name, IFNULL(t1.checked_at, t1.created_at) AS register_time, bus.net_at AS apply_time FROM erui_buyer.buyer t1 LEFT JOIN erui_sys.employee emp ON emp.id = t1.created_by LEFT JOIN erui_buyer.buyer_business bus ON bus.buyer_id = t1.id LEFT JOIN erui_buyer.buyer_agent t2 ON t1.id = t2.buyer_id AND t2.deleted_flag = 'N' LEFT JOIN erui_sys.org_member t3 ON t2.agent_id = t3.employee_id LEFT JOIN erui_sys.org t4 ON (t3.org_id = t4.id AND t4.`status` = 'NORMAL' AND t4.deleted_flag = 'N') WHERE (t1.deleted_flag = 'N' AND buyer_code IS NOT NULL AND t1.`status` IN ('arroved', 'pass'))";
