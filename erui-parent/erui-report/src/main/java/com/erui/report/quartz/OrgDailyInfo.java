...
public class OrgDailyInfo {
	...
	// 合格供应商数量信息sql
	private static final String SUPPLIER_PASS_QUERY_SQL = "SELECT DATE_FORMAT(t4.checked_at, '%Y-%m-%d') AS day_date, t4.name AS org_name , COUNT(1) AS supplier_pass FROM ( SELECT t1.id AS sid , CASE  WHEN t2.name = '易瑞-钻完井设备事业部' THEN '易瑞-钻完井设备事业部' WHEN t2.name = '易瑞-采油工程事业部' THEN '易瑞-采油工程事业部' WHEN t2.name = '易瑞-工业品事业部' THEN '易瑞-工业品事业部' WHEN t2.name = '国内销售部' THEN '易瑞-国内销售部' ELSE '其他' END AS name, t1.checked_at FROM erui_supplier.supplier t1, erui_sys.group t2 WHERE (t1.org_id = t2.id AND t2.deleted_flag = 'N' AND t1.status = 'APPROVED' AND t1.deleted_flag = 'N' AND t1.checked_at IS NOT NULL) ) t4 GROUP BY t4.name, DATE_FORMAT(t4.checked_at, '%Y-%m-%d') ORDER BY 1";
